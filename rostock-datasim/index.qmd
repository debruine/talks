...
R package and [shiny app](https://synthpop.shinyapps.io/synthpop/){target = "_blank"} for generating synthetic versions of sensitive microdata for statistical disclosure control. 
...
- Note non-NA missing values and include as the `cont.na` argument to `syn()`
- Remove variables that are redundant or derivable from others
- Note  variables that depend on other variables. You can set the rules with the parameters `rules` and `rvalues` of the `syn()` function.
